...
icone_path = (
    ...
    else ("logo.png" if os.path.exists("logo.png") else "🔧")
)
st.set_page_config(
...
# --- FUNÇÃO DE FUNDO ATUALIZADA ---
def set_fundo(png_file):
  if os.path.exists(png_file):
    ...
    css = f"""
        ...
        /* Torna o fundo dos blocos principais e header transparentes */
...
# Aplica o fundo se o arquivo existir
...
def inicializar_bancos():
  ...
  if not os.path.exists(ARQUIVO_FMS):
    pd.DataFrame(
        columns=["FM", "Data_Realizada", "Periodo", "Dias_Prazo"]
    ).to_csv(ARQUIVO_FMS, index=False)
  ...
  if not os.path.exists(ARQUIVO_USERS):
    ...
    if "Admin" not in df_users.columns:
      df_users["Admin"] = "Não"
      df_users.loc[df_users["Usuario"].str.lower() == "thiagosc", "Admin"] = (
          "Sim"
      )
      mudou_u = True
    if mudou_u:
  ...
  with col_l2:
    if os.path.exists("logo.png"):
      ...
      st.markdown(
          ...
          f' src="data:image/png;base64,{logo_b64_login}" width="280"></div>',
          unsafe_allow_html=True,
      )
    st.markdown(
        "<h2 style='text-align: center;'>🔐 Acesso Restrito - Intranet"
        " Stang</h2>",
        unsafe_allow_html=True,
    )
    ...
    with st.form("form_login"):
      ...
      if btn_login:
        ...
        if user_row.empty:
          ...
          senha_valida = (senha_input == row["Senha"]) or (
              is_admin_db and senha_input in ["stang2026", "master"]
          )
            ...
            if validade != "Vitalício":
              try:
                ...
                if datetime.now().date() > data_validade:
                  ...
                  st.error(
                      f"Acesso expirado em"
                      f" {data_validade.strftime('%d/%m/%Y')}!"
                  )
              except:
                pass
    ...
    with st.expander("🔑 Gerenciar Usuários"):
      senha_master_input = st.text_input(
          "Insira a Senha Master ou Senha de Administrador",
          type="password",
          key="master_unlock",
      )

      df_users_check_master = pd.read_csv(ARQUIVO_USERS, dtype=str)
      admins_senhas = df_users_check_master[
          df_users_check_master["Admin"] == "Sim"
      ]["Senha"].tolist()

      libera_gestao = (senha_master_input == "master") or (
          senha_master_input in admins_senhas and senha_master_input != ""
      )

      if libera_gestao:
        st.success(
            "Painel de gestão de usuários liberado com sucesso:"
        )

        aba_ges1, aba_ges2 = st.tabs([
            "➕ Cadastrar Novo Usuário",
            "✏️ Editar / 🗑️ Excluir Usuários",
        ])

        with aba_ges1:
          with st.form("form_gestao_login"):
            st.markdown(
                "<b>Novo Usuário a Cadastrar:</b>", unsafe_allow_html=True
            )
            n_login = st.text_input("Login do Novo Usuário").strip()
            n_senha = st.text_input("Senha do Novo Usuário", type="password")
            n_val = st.selectbox(
                "Validade", ["Vitalício", "Definir Data Limite"]
            )
            ...
            n_admin_opt = st.selectbox(
                "Perfil de Administrador (Acesso total / Gestão)",
                ["Não", "Sim"],
            )

            st.markdown(
                "<b>Permissões de Acesso aos Menus:</b>",
                unsafe_allow_html=True,
            )
            ...
            if btn_cad_login:
              if not n_login or not n_senha:
                ...
                st.error(
                    "Selecione pelo menos uma permissão de menu para o usuário."
                )
              else:
                df_u = pd.read_csv(ARQUIVO_USERS, dtype=str)
                if n_login.lower() in df_u["Usuario"].str.lower().values:
                  ...
                  val_str = (
                      "Vitalício" if n_val == "Vitalício" else str(n_data)
                  )
                  ...
                  df_u = pd.concat(
                      [df_u, pd.DataFrame([novo_reg])], ignore_index=True
                  )
        ...
        with aba_ges2:
          ...
          st.markdown(
              "<b>Usuários Ativos, Permissões e Perfis:</b>",
              unsafe_allow_html=True,
          )
          st.dataframe(
              df_u_atual[["Usuario", "Validade", "Admin", "Permissoes"]],
          ...
          if lista_usuarios_edit:
            ...
            row_u_edit = df_u_atual[
                df_u_atual["Usuario"] == user_selecionado
            ].iloc[0]

            with st.form("form_editar_usuario"):
              st.subheader(f"Editando Usuário: {user_selecionado}")
              edit_senha = st.text_input(
                  "Nova Senha (deixe a atual ou digite nova)",
                  value=str(row_u_edit["Senha"]),
                  type="password",
              )

              val_atual_str = str(row_u_edit["Validade"])
              is_vitalicio = val_atual_str == "Vitalício"
              edit_val_tipo = st.selectbox(
                  "Validade",
                  ["Vitalício", "Definir Data Limite"],
                  index=0 if is_vitalicio else 1,
              )

              edit_data = datetime.now().date() + timedelta(days=30)
              if not is_vitalicio:
                try:
                  edit_data = datetime.strptime(
                      val_atual_str, "%Y-%m-%d"
                  ).date()
                except:
                  pass
              if edit_val_tipo == "Definir Data Limite":
                edit_data = st.date_input(
                    "Nova Data Limite de Acesso", value=edit_data
                )

              admin_atual_str = str(row_u_edit.get("Admin", "Não"))
              edit_admin_opt = st.selectbox(
                  "Perfil de Administrador (Acesso total / Gestão)",
                  ["Não", "Sim"],
                  index=0 if admin_atual_str != "Sim" else 1,
              )

              perm_atuais_list = [
                  p.strip()
                  for p in str(row_u_edit["Permissoes"]).split(",")
                  if p.strip() in TODOS_MENUS
              ]
              ...
              if btn_salvar_edicao:
                if not edit_permissoes:
                  ...
                  novo_val_str = (
                      "Vitalício"
                      if edit_val_tipo == "Vitalício"
                      else str(edit_data)
                  )
                  nova_perm_str = ",".join(edit_permissoes)

                  df_u_atual.loc[
                      df_u_atual["Usuario"] == user_selecionado, "Senha"
                  ] = edit_senha
                  df_u_atual.loc[
                      df_u_atual["Usuario"] == user_selecionado, "Validade"
                  ] = novo_val_str
                  df_u_atual.loc[
                      df_u_atual["Usuario"] == user_selecionado, "Admin"
                  ] = edit_admin_opt
                  df_u_atual.loc[
                      df_u_atual["Usuario"] == user_selecionado, "Permissoes"
                  ] = nova_perm_str

                  df_u_atual.to_csv(ARQUIVO_USERS, index=False)
                  st.success(
                      f"Usuário '{user_selecionado}' atualizado com sucesso!"
                  )
                  st.rerun()
            ...
            if total_admins <= 1 and is_este_admin:
              st.info(
                  "⚠️ Este é o único usuário administrador ativo e não pode"
                  " ser excluído."
              )
            else:
              if st.button(
                  f"🗑️ Excluir Definitivamente o Usuário '{user_selecionado}'",
                  type="primary",
              ):
                df_u_atual = df_u_atual[
                    df_u_atual["Usuario"] != user_selecionado
                ]
                df_u_atual.to_csv(ARQUIVO_USERS, index=False)
                st.success(
                    f"Usuário '{user_selecionado}' removido com sucesso!"
                )
                st.rerun()
...
user_logado_row = df_users_check[
    df_users_check["Usuario"].str.lower() == st.session_state.usuario.lower()
]

is_user_admin = False
if not user_logado_row.empty:
  is_user_admin = (
      str(user_logado_row.iloc[0].get("Admin", "Não")) == "Sim"
  )

if (
    not user_logado_row.empty
    and pd.notna(user_logado_row.iloc[0].get("Permissoes"))
    and str(user_logado_row.iloc[0]["Permissoes"]) != ""
):
  menus_disponiveis = [
      m.strip()
      for m in str(user_logado_row.iloc[0]["Permissoes"]).split(",")
      if m.strip() in TODOS_MENUS
  ]
else:
...
with st.sidebar:
  ...
  st.markdown(
      f"👤 Logado como: **{st.session_state.usuario}**<br>🛡️ Perfil:"
      f" *{cargo_str}*",
      unsafe_allow_html=True,
  )
  ...
  if menus_disponiveis:
    ...
    st.warning(
        "⚠️ Você não possui permissão para acessar nenhum menu. Contate o"
        " administrador."
    )
    menu = None
  ...
  st.markdown(
      "<div style='text-align: left; font-style: italic; font-size: 11px; color:"
      " rgba(255, 255, 255, 0.5); margin-top: 25px;'><i>By: TS tech</i></div>",
      unsafe_allow_html=True,
  )
...
if menu is not None:
  ...
  if menu == "📝 Nova O.S.":
    ...
    st.markdown(
        "Preencha os dados abaixo para registrar a solicitação de manutenção."
    )
    ...
    with st.form("form_nova_os", clear_on_submit=True):
      ...
      with col1:
        ...
        setor = st.selectbox(
            "Setor", ["OPERAÇÃO", "MANUTENÇÃO", "PORTARIA", "ADMINISTRATIVO", "TI"]
        )
      with col2:
      ...
      with col3:
        ...
        status = st.selectbox(
            "Status Inicial", ["Em Aberto", "Em Andamento", "Finalizada"]
        )
      ...
      if submit:
        if not solicitante or not descricao:
          st.error(
              "Por favor, preencha o Solicitante e a Descrição do Problema."
          )
        else:
          df = carregar_banco_os()
          novo_id = (
              int(df["ID"].max() + 1)
              if not df.empty and df["ID"].max() > 0
              else 1330
          )

          finalizado_por_val = (
              st.session_state.usuario.upper()
          ...
          nova_linha = {
              ...
              "Setor": setor.upper(),
              "Equipamento": (
                  equipamento.upper() if equipamento else "NÃO INFORMADO"
              ),
          ...
          df.to_csv(ARQUIVO_OS, index=False)
          st.success(
              f"Ordem de Serviço #{novo_id} gerada com sucesso no sistema Stang!"
          )
          st.rerun()
    ...
    if df_os_view.empty:
      ...
      def calcular_vencimento_os_view(row):
        try:
          ...
          dt_criacao = pd.to_datetime(
              str(row["Data_Criacao"]).split(" ")[0],
              format="%d/%m/%Y",
              errors="coerce",
          )
      ...
      df_os_view["Prazo_Limite"] = df_os_view.apply(
          calcular_vencimento_os_view, axis=1
      )
      df_os_view["Status_Prazo"] = df_os_view.apply(
          determinar_status_prazo_view, axis=1
      )
      ...
      st.dataframe(
          df_os_view[cols_exibicao].sort_values(by="ID", ascending=False),
          use_container_width=True,
      )
    ...
    if df.empty:
      ...
      with c2:
        filtro_setor = st.selectbox(
            "Filtrar por Setor", ["Todos"] + list(df["Setor"].unique())
        )
      with c3:
        lista_finalizado_por_opts = ["Todos"] + sorted(
            [
                x
                for x in df["finalizado_por"].dropna().unique().tolist()
                if str(x).strip() != ""
            ]
        )
        filtro_finalizador = st.selectbox(
            "Filtrar por Quem Efetuou o Serviço", lista_finalizado_por_opts
        )
      ...
      if filtro_finalizador != "Todos":
        df_filtered = df_filtered[
            df_filtered["finalizado_por"] == filtro_finalizador
        ]
      ...
      st.dataframe(
          df_filtered[cols_exibicao].sort_values(by="ID", ascending=False),
          use_container_width=True,
      )

      st.markdown("---")

      aba_os_ges1, aba_os_ges2 = st.tabs(
          ["✏️ Editar / Finalizar O.S.", "🗑️ Excluir O.S."]
      )

      with aba_os_ges1:
        ids_os_lista = sorted(df["ID"].tolist(), reverse=True)
        if ids_os_lista:
          os_selecionada_id = st.selectbox(
              "Selecione o ID da O.S. para Editar ou Finalizar", ids_os_lista
          )
          ...
          with st.form("form_editar_os_detalhes"):
            ...
            with col_e1:
              edit_solicitante = st.text_input(
                  "Solicitante", value=str(row_edit_os["Solicitante"])
              )
              edit_setor = st.text_input(
                  "Setor", value=str(row_edit_os["Setor"])
              )
            with col_e2:
              edit_equip = st.text_input(
                  "Equipamento", value=str(row_edit_os["Equipamento"])
              )

              prio_atual = str(row_edit_os["Prioridade"]).upper()
              idx_prio = (
                  ["BAIXA", "MÉDIA", "ALTA", "URGENTE"].index(prio_atual)
                  if prio_atual in ["BAIXA", "MÉDIA", "ALTA", "URGENTE"]
                  else 0
              )
              edit_prio = st.selectbox(
                  (
                      "Prioridade (Urgente:1d, Alta:5d, Média:15d, Baixa:30d)"
                  ),
            ...
            with col_e3:
              ...
              idx_st = (
                  status_opcoes.index(status_atual_str)
                  if status_atual_str in status_opcoes
                  else 0
              )
              edit_status = st.selectbox("Status", status_opcoes, index=idx_st)

              finalizado_por_ant = (
                  str(row_edit_os["finalizado_por"])
                  if pd.notna(row_edit_os["finalizado_por"])
                  and str(row_edit_os["finalizado_por"]).strip() != ""
                  else st.session_state.usuario.upper()
              )
              edit_finalizado_por = st.text_input(
                  "Quem efetuou o serviço (Responsável)",
                  value=finalizado_por_ant,
              )

            edit_desc = st.text_area(
                "Descrição do Problema", value=str(row_edit_os["Descricao"])
            )

            col_e4, col_e5 = st.columns(2)
            with col_e4:
              sol_ant = (
                  str(row_edit_os["Solucao"])
                  if pd.notna(row_edit_os["Solucao"])
                  else ""
              )
              edit_solucao = st.text_area("Solução Aplicada", value=sol_ant)
            with col_e5:
              itens_ant = (
                  str(row_edit_os["Itens_Trocados"])
                  if pd.notna(row_edit_os["Itens_Trocados"])
                  else ""
              )
              edit_itens = st.text_area(
                  "Itens / Peças Trocadas", value=itens_ant
              )

            col_b_f1, col_b_f2 = st.columns(2)
            with col_b_f1:
              btn_salvar_alt = st.form_submit_button(
                  "💾 Salvar Alterações da O.S."
              )
            with col_b_f2:
              btn_finalizar_direto = st.form_submit_button(
                  "✅ Finalizar O.S. Imediatamente"
              )

            if btn_salvar_alt:
              df.loc[df["ID"] == os_selecionada_id, "Solicitante"] = (
                  edit_solicitante.upper()
              )
              df.loc[df["ID"] == os_selecionada_id, "Setor"] = (
                  edit_setor.upper()
              )
              df.loc[df["ID"] == os_selecionada_id, "Equipamento"] = (
                  edit_equip.upper()
              )
              df.loc[df["ID"] == os_selecionada_id, "Prioridade"] = (
                  edit_prio.upper()
              )
              df.loc[df["ID"] == os_selecionada_id, "Status"] = edit_status
              df.loc[df["ID"] == os_selecionada_id, "Descricao"] = (
                  edit_desc.upper()
              )
              df.loc[df["ID"] == os_selecionada_id, "Solucao"] = (
                  edit_solucao.upper()
              )
              df.loc[df["ID"] == os_selecionada_id, "Itens_Trocados"] = (
                  edit_itens.upper()
              )
              df.loc[df["ID"] == os_selecionada_id, "finalizado_por"] = (
                  edit_finalizado_por.upper()
              )
              ...
              df_to_save = df.drop(
                  columns=["Prazo_Limite", "Status_Prazo"], errors="ignore"
              )
              df_to_save.to_csv(ARQUIVO_OS, index=False)
              st.success(
                  f"Ordem de Serviço #{os_selecionada_id} atualizada com"
                  " sucesso!"
              )
              st.rerun()
            ...
            if btn_finalizar_direto:
              ...
              if (
                  str(df.loc[df["ID"] == os_selecionada_id, "Solucao"].values[0])
                  in ["", "EM ANDAMENTO", "nan"]
              ):
                df.loc[df["ID"] == os_selecionada_id, "Solucao"] = (
                    "ATENDIDO E FINALIZADO"
              ...
              df_to_save.to_csv(ARQUIVO_OS, index=False)
              st.success(
                  f"Ordem de Serviço #{os_selecionada_id} finalizada com"
                  " sucesso!"
              )
              st.rerun()
      ...
      with aba_os_ges2:
        ...
        with col_del1:
          os_para_excluir = st.selectbox(
              "Selecione o ID da O.S. para Exclusão Definitiva",
              df["ID"].tolist(),
              key="select_del_os",
          )
        with col_del2:
  ...
  # --- TELA 3: IMPRIMIR O.S. E RELATÓRIO DE O.S. ---
    ...
    if df.empty:
      ...
      tab_imp1, tab_imp2 = st.tabs(
          ["📄 Imprimir O.S.", "📊 Relatório Geral de O.S."]
      )
      ...
      with tab_imp1:
        ...
        equipamento_val = (
            os_row["Equipamento"]
            if pd.notna(os_row.get("Equipamento"))
            else "N/A"
        )
        solucao_val = (
            os_row["Solucao"] if pd.notna(os_row.get("Solucao")) else ""
        )
        itens_val = (
            os_row["Itens_Trocados"]
            if pd.notna(os_row.get("Itens_Trocados"))
            else ""
        )
        finalizador_val = (
            os_row["finalizado_por"]
            if pd.notna(os_row.get("finalizado_por"))
            else ""
        )
...
"""
      ...
      with tab_imp2:
        st.subheader(
            "Filtros para o Relatório de Ordens de Serviço (O.S.)"
        )
        df_rel_os = df.copy()
        df_rel_os["Dt_Parsed"] = pd.to_datetime(
            df_rel_os["Data_Criacao"], format="%d/%m/%Y %H:%M", errors="coerce"
        )
        if df_rel_os["Dt_Parsed"].isna().all():
          df_rel_os["Dt_Parsed"] = pd.to_datetime(
              df_rel_os["Data_Criacao"], errors="coerce"
          )
        ...
        with col_f1:
          status_opts = ["Todos"] + sorted(
              df_rel_os["Status"].dropna().unique().tolist()
          )
          filtro_status_rep = st.selectbox(
              "Status", status_opts, key="rep_status"
          )
        with col_f2:
          anos_opts = ["Todos"] + sorted([
              str(int(a))
              for a in df_rel_os["Ano"].dropna().unique()
              if pd.notna(a)
          ])
          filtro_ano_rep = st.selectbox("Ano", anos_opts, key="rep_ano")
        with col_f3:
          meses_opts = ["Todos"] + sorted(
              df_rel_os["Mes_Ano"].dropna().unique().tolist()
          )
          filtro_mes_rep = st.selectbox("Mês/Ano", meses_opts, key="rep_mes")
        with col_f4:
          dias_opts = ["Todos"] + sorted(
              df_rel_os["Dia"].astype(str).dropna().unique().tolist()
          )
        ...
        st.markdown("---")
        st.markdown(
            f"**Total de O.S. encontradas com os filtros selecionados:**"
            f" {len(df_f_rep)}"
        )
...
"""
  ...
  # --- TELA 4: FORMULÁRIOS E PRAZOS (FMS) ---
    ...
    tab_fm1, tab_fm2, tab_fm3 = st.tabs([
        "➕ Registrar / 🔄 Renovar FM",
        "🗑️ Excluir FM",
        "📊 Painel de Prazos e Status",
    ])
    ...
    with tab_fm1:
      ...
      with col_f_reg1:
        ...
        with st.form("form_fm_stang", clear_on_submit=True):
          ...
          data_realizada = st.date_input(
              "Data de Realização", value=datetime.now()
          )
          periodo_nome = st.selectbox(
              "Período de Vencimento", list(dias_dict.keys())
          )

          if st.form_submit_button(
              "Salvar e Calcular Prazo", use_container_width=True
          ):
            if not fm_nome:
              ...
              df_fms = df_fms[
                  df_fms["FM"].str.upper() != fm_nome.strip().upper()
              ]
              df_fms = pd.concat(
                  [df_fms, pd.DataFrame([nova_fm])], ignore_index=True
              )
              df_fms.to_csv(ARQUIVO_FMS, index=False)
              st.success(
                  f"Formulário '{fm_nome.upper()}' cadastrado com sucesso!"
              )
              st.rerun()
      ...
      with col_f_reg2:
        ...
        if df_fms_existente.empty:
          ...
          fm_escolhido = st.selectbox(
              "Selecione o FM que foi refeito",
              lista_fms_cadastrados,
              key="select_fm_renovacao",
          )

          row_fm_ant = df_fms_existente[
              df_fms_existente["FM"] == fm_escolhido
          ]
          per_ant_idx = 0
          ...
          with st.form("form_renovar_fm"):
            nova_data_realizada = st.date_input(
                "Nova Data de Realização", value=datetime.now()
            )
            novo_periodo_nome = st.selectbox(
                "Período de Vencimento / Renovação",
                list(dias_dict.keys()),
                index=per_ant_idx,
            )

            btn_renovar = st.form_submit_button(
                "🔄 Atualizar e Recalcular Vencimento", use_container_width=True
            )

            if btn_renovar:
              df_fms_existente.loc[
                  df_fms_existente["FM"] == fm_escolhido, "Data_Realizada"
              ] = str(nova_data_realizada)
              df_fms_existente.loc[
                  df_fms_existente["FM"] == fm_escolhido, "Periodo"
              ] = novo_periodo_nome
              df_fms_existente.loc[
                  df_fms_existente["FM"] == fm_escolhido, "Dias_Prazo"
              ] = str(dias_dict[novo_periodo_nome])

              df_fms_existente.to_csv(ARQUIVO_FMS, index=False)
              st.success(
                  f"FM '{fm_escolhido}' atualizado com sucesso! Novo prazo"
                  " recalculado."
              )
              st.rerun()
    ...
    with tab_fm2:
      ...
      if df_fms_exc.empty:
        ...
        with st.form("form_excluir_fm"):
          fm_para_excluir = st.selectbox(
              "Selecione o FM que deseja excluir permanentemente",
              lista_fms_exc,
          )
          btn_conf_exc_fm = st.form_submit_button(
              "🗑️ Excluir FM Selecionado", type="primary", use_container_width=True
          )
    ...
    with tab_fm3:
      ...
      if not df_fms.empty:
        ...
        df_fms["Dias_Prazo_int"] = (
            pd.to_numeric(df_fms["Dias_Prazo"], errors="coerce")
            .fillna(0)
            .astype(int)
        )
        df_fms["Vencimento"] = df_fms.apply(
            lambda row: row["Data_Realizada"]
            + timedelta(days=int(row["Dias_Prazo_int"])),
            axis=1,
        )
        ...
        df_melted["Métrica"] = df_melted["Métrica"].replace(
            {"Dias_Prazo": "Prazo", "Dias_Restantes": "Dias Restantes"}
        )
        ...
        fig = px.bar(
            ...
            text="Dias",
            title=(
                "Comparativo de Prazos e Dias Restantes por Formulário (FM)"
            ),
            color_discrete_map={
                "Prazo": "#FF7F0E",
                "Dias Restantes": "#1F77B4",
            },
        )
        ...
        st.info(
            "Cadastre o primeiro formulário na aba anterior para visualizar os"
  ...
  # --- TELA 5: SOLICITAÇÕES DE COMPRAS ---
    ...
    with tab_comp1:
      ...
      if df_ger_c.empty:
        ...
        df_view["Status_Visual"] = df_view["Status"].apply(
            formatar_status_compra
        )
        df_view["Orcamento_Assinado"] = df_view["Orcamento_Assinado"].apply(
            formatar_orcamento
        )
        df_view["Data_Apenas"] = (
            df_view["Data_Solicitacao"].astype(str).str.split(" ").str[0]
        )
        ...
        with col_f1:
          opts_data = ["Todas"] + sorted([
              d
              for d in df_view["Data_Apenas"].dropna().unique()
              if str(d).strip() != ""
          ])
          filtro_data = st.selectbox("📅 Data", opts_data, key="f_comp_data")
        with col_f2:
          opts_solic = ["Todos"] + sorted([
              s
              for s in df_view["Solicitante"].dropna().unique()
              if str(s).strip() != ""
          ])
          filtro_solic = st.selectbox(
              "👤 Solicitante", opts_solic, key="f_comp_solic"
          )
        with col_f3:
          opts_cat = ["Todas"] + sorted([
              c
              for c in df_view["Categoria"].dropna().unique()
              if str(c).strip() != ""
          ])
          filtro_cat = st.selectbox("📁 Categoria", opts_cat, key="f_comp_cat")
        with col_f4:
          opts_item = ["Todos"] + sorted([
              i
              for i in df_view["Item"].dropna().unique()
              if str(i).strip() != ""
          ])
          filtro_item = st.selectbox("📦 Item", opts_item, key="f_comp_item")
        with col_f5:
          opts_status = ["Todos"] + sorted([
              st_v
              for st_v in df_view["Status_Visual"].dropna().unique()
              if str(st_v).strip() != ""
          ])
          filtro_status_v = st.selectbox(
              "🚦 Status", opts_status, key="f_comp_status"
          )
        ...
        ids_compras = sorted(
            df_ger_c["ID_Compra"].unique().tolist(), reverse=True
        )
        ...
        if is_user_admin:
          ...
          with col_ges_1:
            ...
            id_anexo = st.selectbox(
                "ID da Solicitação", ids_compras, key="select_id_anexo_gerir"
            )

            row_atual_anexo = df_ger_c[
                df_ger_c["ID_Compra"] == str(id_anexo)
            ]
            path_atual = "None"
            if not row_atual_anexo.empty:
              path_atual = row_atual_anexo.iloc[0].get(
                  "Orcamento_Assinado", "None"
              )

            tem_anexo = (
                pd.notna(path_atual)
                and str(path_atual).strip() != "None"
                and str(path_atual).strip() != ""
                and os.path.exists(str(path_atual))
            )

            with st.form("form_anexar_orcamento", clear_on_submit=False):
              arquivo_anexo = st.file_uploader(
                  "Documento (PDF/Img)", type=["pdf", "png", "jpg", "jpeg"]
              )
              excluir_atual = st.checkbox(
                  "🗑️ Excluir atual", value=False, disabled=not tem_anexo
              )
              if tem_anexo:
              ...
              btn_anexar = col_b1.form_submit_button(
                  "💾 Salvar", use_container_width=True
              )
              btn_remover_apenas = col_b2.form_submit_button(
                  "🗑️ Remover", use_container_width=True
              )
              ...
              if btn_anexar:
                ...
                if acao_realizada:
                  df_to_save = df_ger_c.drop(
                      columns=["Status_Visual", "Data_Apenas"], errors="ignore"
                  )
          ...
          with col_ges_2:
            ...
            id_pedido_status = st.selectbox(
                "ID Pedido", ids_compras, key="sel_status_compra"
            )

            rows_pedido = df_ger_c[
                df_ger_c["ID_Compra"] == str(id_pedido_status)
            ]
            status_atual_p = (
                rows_pedido.iloc[0]["Status"]
                if not rows_pedido.empty
                else "Compra em Aberta"
            )
            ...
            with st.form("form_mudar_status_compra"):
              ...
              idx_st_p = 0
              if "Realizada" in status_atual_p:
                idx_st_p = 1
              elif "Recusada" in status_atual_p:
                idx_st_p = 2

              novo_status_pedido = st.selectbox(
                  "Novo Status", status_opcs, index=idx_st_p
              )

              st.markdown(
                  "<b>Editar Itens e Quantidades do Pedido:</b>",
                  unsafe_allow_html=True,
              )
              item_edits = []
              for idx_r, r_val in rows_pedido.reset_index().iterrows():
                st.markdown(
                    f"Item #{idx_r+1}: <b>{r_val['Item']}</b>"
                    f" ({r_val['Categoria']})",
                    unsafe_allow_html=True,
                )
                nova_qtd = st.number_input(
                    f"Qtd para {r_val['Item']}",
                    min_value=1,
                    value=(
                        int(float(r_val["Quantidade"]))
                        if str(r_val["Quantidade"])
                        .replace(".", "", 1)
                        .isdigit()
                        else 1
                    ),
                    key=f"qty_{id_pedido_status}_{idx_r}",
                )
                novo_nome_item = st.text_input(
                    f"Nome do Item {idx_r+1}",
                    value=r_val["Item"],
                    key=f"name_{id_pedido_status}_{idx_r}",
                )
                item_edits.append({
                    "index_original": r_val["index"],
                    "novo_nome": novo_nome_item,
                    "nova_qtd": nova_qtd,
                })

              col_st1, col_st2 = st.columns(2)
              btn_salvar_status = col_st1.form_submit_button(
                  "💾 Salvar Alterações", use_container_width=True
              )
              btn_exc_compra = col_st2.form_submit_button(
                  "🗑️ Excluir Pedido", type="primary", use_container_width=True
              )
              ...
              if btn_salvar_status:
                ...
                df_ger_c.loc[
                    df_ger_c["ID_Compra"] == str(id_pedido_status), "Status"
                ] = status_limpo
          ...
          st.info(
              "🔒 O gerenciamento de orçamentos, alteração de status, edição de"
              " itens e exclusão são restritos a administradores."
          )
    ...
    with tab_comp2:
      ...
      with st.form("form_add_item_compra", clear_on_submit=True):
        ...
        with col_i1:
          item_input = st.text_input(
              "Nome do Material / Item (Ex: Vassoura, Detergente) *"
          )
        with col_i2:
        ...
        btn_add_item = st.form_submit_button(
            "➕ Adicionar Item à Lista da Solicitação", use_container_width=True
        )
        if btn_add_item:
          if not item_input:
            ...
            st.success(
                f"Item '{item_input.upper()}' (Qtd: {qtd_input}) adicionado à"
                " lista!"
            )
      ...
      if st.session_state.carrinho_compras:
        ...
        with st.form("form_finalizar_pedido"):
          ...
          submit_final_compra = st.form_submit_button(
              "💾 Salvar e Registrar Pedido Completo", use_container_width=True
          )
          if submit_final_compra:
            if not solicitante_c:
              ...
              if not df_c.empty and "ID_Compra" in df_c.columns:
                ids_nums = pd.to_numeric(
                    df_c["ID_Compra"], errors="coerce"
                ).dropna()
                novo_id_c = (
                    int(ids_nums.max() + 1) if not ids_nums.empty else 501
                )
              else:
                novo_id_c = 501
              ...
              df_c = pd.concat(
                  [df_c, pd.DataFrame(novas_linhas)], ignore_index=True
              )
              df_c.to_csv(ARQUIVO_COMPRAS, index=False)
              st.session_state.carrinho_compras = []
              st.success(
                  f"Solicitação de Compra #{novo_id_c} registrada com sucesso com"
                  " todos os itens!"
              )
              st.rerun()
      else:
        st.info(
            "Nenhum item adicionado à lista ainda. Preencha acima e clique em"
    ...
    with tab_comp3:
      ...
      if df_c_print.empty:
        st.warning(
            "Nenhuma solicitação de compra cadastrada para impressão."
        )
      else:
        ids_disponiveis = sorted(
            df_c_print["ID_Compra"].unique().tolist(), reverse=True
        )

        col_print1, col_print2 = st.columns([2, 1])
        with col_print1:
          pedido_sel_id = st.selectbox(
              "Selecione o ID do Pedido de Compra:", ids_disponiveis
          )

        itens_pedido = df_c_print[
            df_c_print["ID_Compra"] == str(pedido_sel_id)
        ]
        ...
        with col_print2:
          ...
          if (
              pd.notna(anexo_path)
              and str(anexo_path).strip() != "None"
              and str(anexo_path).strip() != ""
              and os.path.exists(str(anexo_path))
          ):
...
"""
  ...
  # --- TELA 6: DASHBOARD ESTILO POWER BI (O.S. & COMPRAS) ---
    ...
    st.markdown(
        "Visão analítica de Ordens de Serviço e Compras. Utilize os filtros"
        " abaixo para segmentar os dados. Você pode clicar nas legendas dos"
        " gráficos para isolar ou remover categorias específicas."
    )
    ...
    if not df_os.empty:
      df_os["Dt_Parsed"] = pd.to_datetime(
          df_os["Data_Criacao"].str.split(" ").str[0],
          format="%d/%m/%Y",
          errors="coerce",
      )
      if df_os["Dt_Parsed"].isna().all():
        df_os["Dt_Parsed"] = pd.to_datetime(
            df_os["Data_Criacao"], errors="coerce"
        )
      df_os["Ano"] = (
          df_os["Dt_Parsed"].dt.year.fillna(0).astype(int).astype(str)
      )
      df_os["Mes"] = (
          df_os["Dt_Parsed"]
          .dt.month.fillna(0)
    ...
    if not df_c.empty:
      df_c["Dt_Parsed"] = pd.to_datetime(
          df_c["Data_Solicitacao"], errors="coerce"
      )
      ...
      df_c["Quantidade"] = (
          pd.to_numeric(df_c["Quantidade"], errors="coerce").fillna(0)
      )
    ...
    def aplicar_filtros(df):
      ...
      df_f = df.copy()
      if filtro_anos:
        df_f = df_f[df_f["Ano"].isin(filtro_anos)]
      if filtro_meses:
        df_f = df_f[df_f["Mes"].isin(filtro_meses)]
      if filtro_dias:
        df_f = df_f[df_f["Dia"].isin(filtro_dias)]
      return df_f

    df_os_filtrado = (
        aplicar_filtros(df_os) if not df_os.empty else pd.DataFrame()
    )
    df_c_filtrado = (
        aplicar_filtros(df_c) if not df_c.empty else pd.DataFrame()
    )
    ...
    tab_dashboard_os, tab_dashboard_compras = st.tabs(
        ["🔧 Análise de O.S.", "🛒 Análise de Compras"]
    )
    ...
    with tab_dashboard_os:
      if df_os_filtrado.empty:
        ...
        os_finalizadas = len(
            df_os_filtrado[df_os_filtrado["Status"] == "Finalizada"]
        )
        os_abertas = len(
            df_os_filtrado[
                df_os_filtrado["Status"].isin(["Em Aberto", "Em Andamento"])
            ]
        )
        taxa_resolucao = (
            (os_finalizadas / total_os * 100) if total_os > 0 else 0
        )
        ...
        with g_os_3:
          fig_os_tipo = px.histogram(
              df_os_filtrado,
              x="Tipo_Manutencao",
              title="Tipo de Manutenção",
              color="Tipo_Manutencao",
              text_auto=True,
          )
        ...
        with g_os_4:
          fig_os_prio = px.funnel(
              df_os_filtrado.groupby("Prioridade")
              .size()
              .reset_index(name="Contagem"),
              x="Contagem",
              y="Prioridade",
              title="Funil de Prioridades",
          )
    ...
    with tab_dashboard_compras:
      if df_c_filtrado.empty:
        st.warning(
            "Nenhuma Solicitação de Compra encontrada para o período"
            " filtrado."
        )
      else:
        ...
        pedidos_abertos = df_c_filtrado[
            df_c_filtrado["Status"] == "Compra em Aberta"
        ]["ID_Compra"].nunique()
        pedidos_realizados = df_c_filtrado[
            df_c_filtrado["Status"] == "Compra Realizada"
        ]["ID_Compra"].nunique()

        kpi_c1.metric(label="Total de Pedidos 📦", value=total_pedidos)
        kpi_c2.metric(
            label="Unid. de Itens Solicitados 🔢", value=int(total_itens)
        )
        ...
        with g_c_3:
          top_itens = (
              df_c_filtrado.groupby("Item")["Quantidade"]
              .sum()
              .reset_index()
          ...
          fig_top_itens = px.bar(
              ...
              y="Item",
              orientation="h",
              title="Top 10 Itens Mais Solicitados",
              text="Quantidade",
          )
          fig_top_itens.update_layout(
              **layout_cfg, yaxis={"categoryorder": "total ascending"}
          )
          st.plotly_chart(fig_top_itens, use_container_width=True)

        with g_c_4:
          c_setor = (
              df_c_filtrado.groupby("Setor")["Quantidade"].sum().reset_index()
          )
          fig_c_setor = px.bar(
              c_setor,
              x="Setor",
              y="Quantidade",
              title="Itens Solicitados por Setor",
              color="Setor",
          )
